...
if __name__ == "__main__":
    ...
    print("build content index for BIRD's new training set databases...")
    remove_contents_of_a_folder("./data/data_synthesis_split/bird/new_train/db_contents_index")
    # build content index for BIRD's new training set databases
    for db_id in os.listdir("./data/data_synthesis_split/bird/new_train/train_databases"):
        if db_id.endswith(".json"):
            continue
        print(db_id)
        build_content_index(
            os.path.join("./data/data_synthesis_split/bird/new_train/train_databases/", db_id, db_id + ".sqlite"),
            os.path.join("./data/data_synthesis_split/bird/new_train/db_contents_index/", db_id)
        )

    print("build content index for BIRD's new dev set databases...")
    remove_contents_of_a_folder("./data/data_synthesis_split/bird/new_dev/db_contents_index")
    # build content index for BIRD's new dev set databases
    for db_id in os.listdir("./data/data_synthesis_split/bird/new_dev/dev_databases"):
        if db_id.endswith(".json"):
            continue
        print(db_id)
        build_content_index(
            os.path.join("./data/data_synthesis_split/bird/new_dev/dev_databases/", db_id, db_id + ".sqlite"),
            os.path.join("./data/data_synthesis_split/bird/new_dev/db_contents_index/", db_id)
        )
